  ...
  <div class="layout">
    ...
    >
      <sidebar
        class="sidebar"
        v-show="getIsUnFold"
        :style="{ width: sidbarWidth }"
      />
    </transition>
    <div
      class="right"
      :style="{ 'margin-left': getIsUnFold ? sidbarWidth : 0 }"
    >
      <transition name="fold-top">
        <head-bar
          v-show="getIsUnFold"
          class="head-bar"
          :style="{ left: sidbarWidth }"
        />
      </transition>
      <app-main
        class="app-main"
        :class="{ 'app-main--isUnFold': !getIsUnFold }"
      />
...
watch(
  ...
  (val) =>
    (copyright.value = val === undefined ? settingStore.showCopyright : val),
  { immediate: true }
);

const getIsUnFold = computed(
  () => settingStore.menuSetting.show && settingStore.headerSetting.show
);
const collapse = computed(() => settingStore.collapse);

//是否有主侧边栏
const isMainMenu = computed(
  () => menuStore.allRoutes > 1 || settingStore.alwaysShowMainSidebar
);

const sidbarWidth = computed(() => {
  if (collapse.value) {
    //有主侧边栏的收起宽度
    return isMainMenu.value
      ? variables.sidbarCollapseWidth
      : variables.navmenuCollapseWidth;
  } else {
    //有主侧边栏的展开宽度
